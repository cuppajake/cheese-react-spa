...
class MenuView extends Component {
  ...
  addToCheeses = cheese =>
    this.setState(state => {
      const { menu } = state;
      const updatedCheeses = [cheese, ...menu.cheeses];
      // update state by merging the menu data with a new merged cheeses property
      return { menu: { ...menu, cheeses: updatedCheeses } };
    });
  ...
  removeFromCheeses = cheeseID =>
    this.setState(state => {
      ...
      const updatedCheeses = menu.cheeses.filter( (cheese) => {return cheese.id !== cheeseID} );
  ...
  render() {
    ...
    return (
      <Container>
        ...
        <Row>
          <Col>
            <AddMenuCheeseForm addCheese={this.addToCheeses} menuID={menu.id} currentCheeses={menu.cheeses}/>
          </Col>
        </Row>
        <hr />
        <Row>
          <Col>
            <CheesesList cheeses={menu.cheeses}
